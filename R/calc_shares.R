...
#' @param group Bare column name where groups are given
...
calc_shares <- function(df, ..., group = variable, denom = "Total", estimate = estimate, moe = NULL) {
  ...
  if (!rlang::quo_is_null(rlang::enquo(moe))) {
    ...
    bound <- dplyr::bind_rows(
      calcs %>%
        dplyr::select(!!!join_cols, !!quo_name(est_name) := total_est, !!quo_name(moe_name) := total_moe) %>%
        dplyr::mutate(variable = denom) %>%
        unique(),
