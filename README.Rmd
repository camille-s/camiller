...
edu_rates %>%
  ...
  mutate(variable = fct_relabel(variable, function(x) stringr::str_replace_all(x, "_", " ") %>% cap_first()) %>%
           fct_recode("Bachelor's or higher" = "Bachelors plus", "Some college or Associate's" = "Some college or aa") %>%
           fct_rev()) %>%
  ggplot(aes(x = name, y = share, fill = variable)) +
    ...
    labs(x = NULL, y = "Share of adults 25+", fill = "Education level", title = "Educational attainment level by town", subtitle = "Among adults ages 25 or over, 2016", caption = "Source: US Census Bureau ACS 2016 5-year estimates") +
